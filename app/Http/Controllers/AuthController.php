...
class AuthController extends Controller
{
    public function login(Request $request)
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            // Si es admin, darle acceso completo a todas las secciones de permisos
            if ($isAdmin) {
                // Obtener todas las secciones de permisos disponibles
    ...
    {
        // Si el usuario está autenticado con sanctum, revocar todos sus tokens
        if ($request->user()) {
            $request->user()->tokens()->delete();
        }
        
        // También cerrar sesión si existe
        if (Auth::check()) {
            Auth::logout();
        }
