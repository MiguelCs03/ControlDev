            ...
            <VRow>
              ...
              >
                <VTextField
                  ...
                  :append-inner-icon="isNewUserPasswordVisible ? 'tabler-eye-off' : 'tabler-eye'"
                  @click:append-inner="isNewUserPasswordVisible = !isNewUserPasswordVisible"
                />
              </VCol>
        ...
        <VCardText>
          <VRow>
            ...
            >
              ...
              >
                <VImg
                  v-if="usuarioDetalle.avatar_url"
                  :src="usuarioDetalle.avatar_url"
                />
                <span
                  v-else
                  class="text-h4"
                >{{ obtenerIniciales(usuarioDetalle) }}</span>
              </VAvatar>
              <h3 class="mt-4">
                {{ usuarioDetalle.name }}
              </h3>
            ...
            <VCol cols="6">
              ...
              <div class="text-body-1">
                {{ usuarioDetalle.username || 'No especificado' }}
              </div>
            </VCol>
            ...
            <VCol cols="6">
              ...
              <div class="text-body-1">
                {{ usuarioDetalle.numero || 'No especificado' }}
              </div>
            </VCol>
            ...
            <VCol cols="6">
              ...
              <div class="text-body-1">
                {{ usuarioDetalle.cargo || 'No especificado' }}
              </div>
            </VCol>
            ...
            <VCol cols="6">
              ...
              <div class="text-body-1">
                {{ usuarioDetalle.fecha_nacimiento || 'No especificada' }}
              </div>
            </VCol>
            ...
            <VCol cols="6">
              ...
              <div class="text-body-1">
                {{ formatearFecha(usuarioDetalle.created_at) }}
              </div>
            </VCol>
            ...
            <VCol cols="12">
              ...
              <div class="text-body-1">
                {{ formatearFecha(usuarioDetalle.updated_at) }}
              </div>
            </VCol>
          </VRow>
        </VCardText>
...
// Opciones para el select de roles (computed desde API)
const roleOptions = computed(() => {
  const options = [{ title: 'Todos', value: null }]
  if (Array.isArray(roles.value)) {
    roles.value.forEach(role => {
      options.push({ title: role.nombre, value: role.nombre })
    })
  }
...
const guardarNuevoUsuario = async () => {
  ...
  try {
    ...
    resetForm()
    notificationStore.addNotification(`Usuario ${response.data.name} creado exitosamente`, 'success')
  } catch (error) {
...
const toggleEstadoUsuario = async usuario => {
  try {
    ...
    notificationStore.addNotification(`Usuario ${usuario.name} ${accion} correctamente`, 'success')
  } catch (error) {
    console.error('Error al cambiar estado del usuario:', error)
    notificationStore.addNotification('Error al cambiar el estado del usuario', 'error')
  }
}
...
/* Reducir espacio superior de la página para esta vista */
.page-compact {
  /* Pull page up to reduce top gap (adjust value as needed) */
  margin-top: -12px !important;
  padding-top: 0 !important;
}

.page-compact > .v-card:first-child {
